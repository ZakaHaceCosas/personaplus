...
export async function getDefaultLocale(): Promise<"es" | "en"> {
    try {
        ...
        const savedData: string | null = await AsyncStorage.getItem(StoredItemNames.userData);
        ...
        try {
            ...
            logToConsole("Error parsing JSON from AsyncStorage: " + e, "warn", {
                ...
                isHandler: false,
                function: "getDefaultLocale() @ try-catch #1 @ sub try-catch #1"
            }, false);
        }

        const savedLanguageValue: "es" | "en" = savedDataValue?.language ?? "es"; // TODO: change to "en" upon release
...
async function changeLanguage(language: "es" | "en"): Promise<0 | 1> {
    try {
        ...
        await AsyncStorage.setItem(StoredItemNames.userData, JSON.stringify({ ...userData, language }));
