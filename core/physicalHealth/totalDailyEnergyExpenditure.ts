...
export default function calculateTotalDailyEnergyExpenditure(age: number, gender: "male" | "female", weight: number, height: number, activeness: "poor" | "light" | "moderate" | "intense" | "extreme", provideContext?: boolean, provideExplanation?: boolean): CoreLibraryResponse {
    const bmrSource: CoreLibraryResponse = CoreLibrary.physicalHealth.BasalMetabolicRate.calculate(age, gender, weight, height, activeness, true, true)
    ...
    if (activeness === "poor") {
        calc = bmr * 1.2;
        context = "The recommended daily caloric ingest would be of " + calc + ", taking into account the specified level of activeness is " + activeness + " and the BMR is " + bmr;
